...
S3_MODEL_BASE = f"s3://{S3_BUCKET}/models/rf"
...
def create_rf_pipeline(feature_cols):
    """
    Feature engineering + RandomForest model.
...
def main():
    spark = create_spark_session("RF_Training")

    # 1. Load and prepare data
    df, feature_cols = load_and_prepare_data(spark)

    # 2. Train / test split (same seed as LR; also overwrite same processed paths)
    train_df, test_df = df.randomSplit([0.8, 0.2], seed=42)
    print(f"Train size: {train_df.count():,} | Test size: {test_df.count():,}")

    # Optional: re-write train/test (same split as LR)
    ...
    pipeline = create_rf_pipeline(feature_cols)
